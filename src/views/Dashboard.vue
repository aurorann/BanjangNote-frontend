...
const fetchProjectsForCalendar = async () => {
  loading.value = true
  try {
    // 1. 달력 화면에 보이는 '첫 번째 일요일' 구하기 (예: 4월 26일)
    const firstDayOfMonth = new Date(currentYear.value, currentMonth.value - 1, 1)
    const gridStart = new Date(firstDayOfMonth)
    gridStart.setDate(gridStart.getDate() - gridStart.getDay())

    // 2. 달력 화면에 보이는 '마지막 토요일' 구하기 (예: 6월 6일)
    const lastDayOfMonth = new Date(currentYear.value, currentMonth.value, 0)
    const gridEnd = new Date(lastDayOfMonth)
    gridEnd.setDate(gridEnd.getDate() + (6 - gridEnd.getDay()))

    // 3. YYYY-MM-DD 포맷팅 함수 (한국 시간 기준 안전하게 변환)
    const formatDate = (d) => `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`

    const query = new URLSearchParams()
    if (appliedFilter.value.clientId) query.append('clientId', appliedFilter.value.clientId)

    // 🔥 5월 1일 ~ 31일이 아니라, 달력의 실제 그리드 기간(4/26 ~ 6/6)을 서버에 요청
...
onMounted(async () => {
  try {
    ...
    if (viewMode.value === 'calendar') {
      await fetchProjectsForCalendar()
    } else {
      await fetchProjects(true)
    }

    observer = new IntersectionObserver((entries) => {
      // 뷰 모드가 card일 때만 무한 스크롤 작동
